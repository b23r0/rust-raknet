...
impl FrameSetPacket {
    ...
    pub async fn _deserialize(buf : Vec<u8>) -> Result<(Self , bool)>{
...
impl FrameVec {
    pub async fn new(buf : Vec<u8>) -> Result<Self>{
    ...
    pub async fn _serialize(&self) -> Result<Vec<u8>>{
        let mut writer = RaknetWriter::new();
...
impl SendQ{
    ...
    pub fn nack(&mut self , sequence : u32 , tick : i64){
        if self.packets.contains_key(&sequence){
            let p = self.packets.get(&sequence).unwrap();
    ...
    pub fn flush(&mut self) -> Vec<FrameSetPacket> {
        ...
        if !self.repeat_request.is_empty(){
            let keys : Vec<u32> = self.repeat_request.keys().cloned().collect();
            for i in keys{
                let frame = &mut self.repeat_request.get_mut(&i).unwrap().0;
                frame.sequence_number = self.sequence_number;
                self.sequence_number += 1;
...
async fn test_frame_serialize_deserialize(){
    ...
    let a = FrameSetPacket::_deserialize(p.clone()).await.unwrap();
    assert!(a.0.serialize().await.unwrap() == p);
...
async fn test_sendq(){
    ...
    s.insert(Reliability::Reliable, &p.serialize().await.unwrap(), 0);

    let p = FrameSetPacket::new(Reliability::Reliable, vec![]);
    s.insert(Reliability::Reliable, &p.serialize().await.unwrap(), 50);
...
async fn test_client_packet1(){
    ...
    let b = FrameVec::new(a.to_vec()).await.unwrap();
